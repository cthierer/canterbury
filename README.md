...
| `required key VAULT_SERVICE_ROOT missing value`              | `.env` or the shell environment is missing the vault service root path.                                    | Set `VAULT_SERVICE_ROOT` to `./sample-vault` for the demo or to your mirrored vault path.                                                             |
| `required key VAULT_SERVICE_AUTH_ISSUER missing value`       | `.env` or the shell environment is missing the expected JWT issuer.                                        | Set `VAULT_SERVICE_AUTH_ISSUER` to the issuer used by your auth provider or local dev-auth service.                                                   |
| `required key VAULT_SERVICE_AUTH_AUDIENCE missing value`     | `.env` or the shell environment is missing the expected JWT audience.                                      | Set `VAULT_SERVICE_AUTH_AUDIENCE`, and mint/request tokens with the same audience.                                                                    |
| `required key VAULT_SERVICE_AUTH_JWKS_URL missing value`     | `.env` or the shell environment is missing the JWKS URL.                                                   | Set `VAULT_SERVICE_AUTH_JWKS_URL`, such as `http://127.0.0.1:50052/.well-known/jwks.json` for local development.                                      |
| `required key VAULT_SERVICE_AUTH_MAPPING_FILE missing value` | `.env` or the shell environment is missing the scope mapping path.                                         | Set `VAULT_SERVICE_AUTH_MAPPING_FILE` to a TOML mapping file such as `./sample-auth/scopes.toml`.                                                     |
| `required key VAULT_SERVICE_AUDIT_ROOT missing value`        | `.env` or the shell environment is missing the audit log directory.                                        | Set `VAULT_SERVICE_AUDIT_ROOT` to an explicit directory outside the vault, such as `./audit`.                                                         |
| `required key VAULT_SERVICE_AUDIT_HMAC_KEY missing value`    | `.env` or the shell environment is missing the audit HMAC key.                                             | Generate a literal key with `openssl rand -base64 32` and set `VAULT_SERVICE_AUDIT_HMAC_KEY`.                                                         |
| `HMAC key must be base64 encoded`                            | `VAULT_SERVICE_AUDIT_HMAC_KEY` is not base64.                                                              | Replace it with a literal key generated by `openssl rand -base64 32`.                                                                                 |
| `HMAC key must decode to at least 32 bytes`                  | `VAULT_SERVICE_AUDIT_HMAC_KEY` decodes to fewer than 32 bytes.                                             | Replace it with a literal key generated by `openssl rand -base64 32`.                                                                                 |
